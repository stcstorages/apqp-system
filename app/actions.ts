...
// HELPER: Auto-Calculate Gantt
async function recalculateParent(supabase: any, parentId: string | null, projectId: string) {
  if (!parentId || parentId === 'none') return
  const { data: children } = await supabase.from('gantt_tasks').select('start_date, end_date, progress').eq('parent_id', parentId)
...
export async function createProject(formData: FormData) {
  ...
  if (projError || !newProject) return { error: projError?.message || 'Failed to create project' }

  // Template Copying
  if (!is_template_creation && (!source_id || source_id === 'none')) {
    const { data: custTemplate } = await supabase.from('projects').select('id').eq('is_template', true).eq('customer', customer).eq('product_type', product_type).single()
    if (custTemplate) source_id = custTemplate.id
    else {
      const { data: sibTemplate } = await supabase.from('projects').select('id').eq('is_template', true).eq('customer', 'SIB').eq('product_type', product_type).single()
  ...
  if (source_id && source_id !== 'none') {
    const { data: sourceSteps } = await supabase.from('process_steps').select(`*, pfmea_records (*, control_plan_records (*))`).eq('project_id', source_id).order('step_number', { ascending: true })
    if (sourceSteps) {
      for (const step of sourceSteps) {
        const { data: newStep } = await supabase.from('process_steps').insert({
          project_id: newProject.id, step_number: step.step_number, description: step.description, symbol_type: step.symbol_type, special_char_id: step.special_char_id, remarks: step.remarks, machine_tools: step.machine_tools
        }).select().single()
        if (!newStep) continue;
        for (const risk of step.pfmea_records) {
          const { data: newRisk } = await supabase.from('pfmea_records').insert({
            step_id: newStep.id, failure_mode: risk.failure_mode, failure_effect: risk.failure_effect, severity: risk.severity, special_char_id: risk.special_char_id, cause: risk.cause, control_prevention: risk.control_prevention, occurrence: risk.occurrence, current_controls: risk.current_controls, detection: risk.detection, recommended_actions: risk.recommended_actions, responsibility: risk.responsibility 
          }).select().single()
          if (!newRisk) continue;
          for (const cp of risk.control_plan_records) {
             await supabase.from('control_plan_records').insert({
               pfmea_id: newRisk.id, characteristic_product: cp.characteristic_product, characteristic_process: cp.characteristic_process, specification_tolerance: cp.specification_tolerance, eval_measurement_technique: cp.eval_measurement_technique, sample_size: cp.sample_size, sample_freq: cp.sample_freq, control_method: cp.control_method, reaction_plan: cp.reaction_plan, reaction_owner: cp.reaction_owner
             })
          }
        }
      }
...
// Reverted: Simple Add Customer without Logo URL
...
export async function updateProjectDetails(formData: FormData) {
  ...
  const updates: any = {}
  formData.forEach((value, key) => { if(key !== 'project_id' && !key.startsWith('$')) updates[key] = value === '' ? null : value })
...
// ... (Rest of FMEA, Process, Gantt functions remain identical to before)
// I am including them in shorthand here for brevity, but you must keep the existing ones
// or copy the full file from the previous step if you want to be 100% sure.
// Since you requested FULL CODE, I will paste the rest below to ensure no copy-paste errors.
...
export async function addFmeaRow(formData: FormData) {
  ...
  await supabase.from('pfmea_records').insert({ step_id: stepId, failure_mode, failure_effect, severity, special_char_id: specialCharId, cause, control_prevention, occurrence, current_controls, detection, recommended_actions, responsibility, action_taken, act_severity, act_occurrence, act_detection })
  revalidatePath(`/projects/${projectId}/fmea`)
}
export async function updateFmeaRow(formData: FormData) {
  ...
  const act_occurrence = parseInt(formData.get('act_occurrence') as string) || null
  const act_detection = parseInt(formData.get('act_detection') as string) || null
  await supabase.from('pfmea_records').update({ failure_mode, failure_effect, severity, special_char_id: specialCharId, cause, control_prevention, occurrence, current_controls, detection, recommended_actions, responsibility, action_taken, act_severity, act_occurrence, act_detection }).eq('id', id)
  revalidatePath(`/projects/${projectId}/fmea`)
}
...
export async function addGanttTask(formData: FormData) {
  ...
  if (type === 'project' && (!start || !end)) { const today = new Date().toISOString(); start = today; end = today; }
  const { data: maxOrder } = await supabase.from('gantt_tasks').select('order_index').eq('project_id', projectId).order('order_index', { ascending: false }).limit(1).single()
  const nextOrder = (maxOrder?.order_index || 0) + 1
  await supabase.from('gantt_tasks').insert({ project_id: projectId, name, start_date: new Date(start).toISOString(), end_date: new Date(end).toISOString(), progress: 0, type: type || 'task', parent_id: parentId === 'none' ? null : parentId, order_index: nextOrder })
  if (parentId && parentId !== 'none') await recalculateParent(supabase, parentId, projectId)
  revalidatePath(`/projects/${projectId}/gantt`)
}
export async function updateGanttTaskDetails(formData: FormData) {
  ...
  const type = formData.get('type') as string
  const parentId = formData.get('parent_id') as string
  await supabase.from('gantt_tasks').update({ name, start_date: new Date(start).toISOString(), end_date: new Date(end).toISOString(), progress: parseInt(progress), type, parent_id: parentId === 'none' ? null : parentId }).eq('id', id)
  if (parentId && parentId !== 'none') await recalculateParent(supabase, parentId, projectId)
  revalidatePath(`/projects/${projectId}/gantt`)
}
export async function deleteGanttTask(formData: FormData) {
  ...
  if (task?.parent_id) await recalculateParent(supabase, task.parent_id, projectId)
  revalidatePath(`/projects/${projectId}/gantt`)
}
...
export async function moveGanttTask(formData: FormData) {
  ...
  const projectId = formData.get('project_id') as string
  const { data: allTasks } = await supabase.from('gantt_tasks').select('*').eq('project_id', projectId).order('order_index', { ascending: true })
  if (!allTasks) return
  ...
  if (movingTask.type === 'project') {
    for (let i = currentIndex + 1; i < allTasks.length; i++) {
      if (allTasks[i].parent_id === movingTask.id) { movingBlockIds.push(allTasks[i].id) } else { break }
    }
  }
  ...
  if (swapTargetIndex !== -1) {
    ...
    if (direction === 'down') { if (swapTargetIndex > currentIndex) insertPos = swapTargetIndex - movingBlockIds.length + 1 }
    newOrder.splice(insertPos, 0, ...block)
    for (let i = 0; i < newOrder.length; i++) { await supabase.from('gantt_tasks').update({ order_index: i }).eq('id', newOrder[i].id) }
  }
  revalidatePath(`/projects/${projectId}/gantt`)
}