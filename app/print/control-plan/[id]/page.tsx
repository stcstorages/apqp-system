...
export default async function ControlPlanPrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 2. Safe Data Fetch (Deep query but NO JOIN on Special Char or complex nest that might fail)
  ...
  // 3. Fetch Library for JS Matching
  ...
  return (
    <div className="min-h-screen bg-white text-black p-4 print-container">
      ...
      <div className="mb-2 text-xs border border-black p-1">
          <div className="font-bold text-center border-b border-black pb-1 mb-1">CONTROL PLAN ({project.cp_phase || 'General'})</div>
          <div className="grid grid-cols-3 gap-4 text-[9px]">
             <div><strong>No:</strong> {project.cp_number || '-'}</div>
             <div><strong>Part:</strong> {project.part_number}</div>
             <div><strong>Date:</strong> {formatDate(project.cp_date_orig)}</div>
          </div>
      </div>
