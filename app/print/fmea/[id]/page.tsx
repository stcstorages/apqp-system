...
export default async function FmeaPrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-white text-black p-4 print-container">
      <style>{`
        @media print { @page { size: landscape; margin: 5mm; } body { -webkit-print-color-adjust: exact; print-color-adjust: exact; font-family: Arial, sans-serif; } .print-border-black { border-color: #000 !important; } table { font-size: 8px; } }
      `}</style>
      ...
      <div className="mb-2 text-xs">
        ...
        <div className="border border-black flex">
           ...
           <div className="w-1/3 border-r border-black">
              <div className="border-b border-black p-1 h-8"><div className="text-[8px] text-gray-500">Process Responsibility</div><div>{project.key_contact || '-'}</div></div>
      ...
      <table className="w-full border-collapse border border-black">
        <thead>
          <tr className="bg-gray-100 text-center font-bold">
            <th className="border border-black p-1" rowSpan={2}>Step / Process</th>
            <th className="border border-black p-1" rowSpan={2}>Failure Mode</th>
            <th className="border border-black p-1" rowSpan={2}>Failure Effects</th>
            <th className="border border-black p-1 w-6" rowSpan={2}>Sev</th>
            <th className="border border-black p-1 w-6" rowSpan={2}>Cls</th>
            <th className="border border-black p-1" rowSpan={2}>Potential Cause</th>
            <th className="border border-black p-1" rowSpan={2}>Prevention</th>
            <th className="border border-black p-1 w-6" rowSpan={2}>Occ</th>
            <th className="border border-black p-1" rowSpan={2}>Detection</th>
