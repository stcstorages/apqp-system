...
export default async function GanttPrintPage({
  ...
  // Logic: Recalculate Headers for Print
  ...
  return (
    <div className="min-h-screen bg-white text-gray-800 text-[10px] font-sans">
            ...
            {weeks.map((w, i) => {
              ...
              return (
                <div key={i} className="absolute top-0 bottom-0 border-l border-gray-200 pl-1" style={{ left: `${left}%` }}>
                   {isNewMonth && <div className="text-[9px] font-bold text-gray-800 uppercase absolute top-0 left-1">{w.toLocaleDateString('en-US', { month: 'short', year: '2-digit' })}</div>}
                ...
                return processedTasks?.map((task) => {
                   ...
                   return (
                     <div key={task.id} className={`h-5 relative w-full ${isHeader ? 'border-b border-gray-100/50' : ''}`}>
                        {isHeader && (
                          <div className="absolute top-1" style={{ left: `${left}%`, width: `${width}%` }}>
                             <div className="h-3 bg-gray-300 rounded-md border-l-4 border-gray-500 absolute w-full"></div>
                             <span className="absolute left-full ml-1 top-0 text-[8px] font-bold text-gray-600 whitespace-nowrap leading-3">{task.name}</span>
                          </div>
                        )}
                        {isMilestone && (
                          <>
                        ...
                        {!isHeader && !isMilestone && (
                          <>
                            <div className={`absolute top-1 h-3 rounded-full shadow-sm flex items-center overflow-hidden ${colorTheme.bar} bg-opacity-30 border ${colorTheme.border}`} style={{ left: `${left}%`, width: `${Math.max(width, 0.5)}%` }}>
      ...
      <div className="mt-2 text-center text-gray-400 text-[9px]">Generated by SIB APQP System • {new Date().toLocaleDateString()}</div>
    </div>
  )
}