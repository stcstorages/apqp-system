...
export default async function ProjectHub({
  ...
  return (
    <div className="space-y-8">
      {/* Top Section: Global Project Settings */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Link href={`/projects/${id}/process-flow`} className="group block">
          <div className="h-full bg-white rounded-lg shadow border border-gray-200 p-6 hover:border-blue-500 hover:shadow-md transition cursor-pointer">
            ...
            <p className="text-gray-600 text-sm mb-4">Define manufacturing steps, flow sequence, and assign symbols.</p>
            <span className="inline-block bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded">{stepCount || 0} Steps Defined</span>
          </div>
        </Link>
        ...
        <Link href={`/projects/${id}/fmea`} className="group block">
          <div className="h-full bg-white rounded-lg shadow border border-gray-200 p-6 hover:border-red-500 hover:shadow-md transition cursor-pointer">
            ...
            <p className="text-gray-600 text-sm mb-4">Identify failure modes, calculate RPN, and plan corrective actions.</p>
            <span className="inline-block bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded">AIAG Standard</span>
          </div>
        </Link>
        ...
        <Link href={`/projects/${id}/control-plan`} className="group block">
          <div className="h-full bg-white rounded-lg shadow border border-gray-200 p-6 hover:border-green-500 hover:shadow-md transition cursor-pointer">
            ...
            <p className="text-gray-600 text-sm mb-4">Define product characteristics, evaluation methods, and reaction plans.</p>
            <span className="inline-block bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded">Production Ready</span>
          </div>
        </Link>
        ...
        <Link href={`/projects/${id}/gantt`} className="group block">
          <div className="h-full bg-white rounded-lg shadow border border-gray-200 p-6 hover:border-purple-500 hover:shadow-md transition cursor-pointer">
            ...
            <p className="text-gray-600 text-sm mb-4">Manage project timeline, milestones, and critical path.</p>
            <span className="inline-block bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded">{taskCount || 0} Tasks Active</span>
          </div>
        </Link>
      </div>
    </div>
