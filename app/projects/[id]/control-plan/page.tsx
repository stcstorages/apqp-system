      ...
      {steps?.map((step) => (
        <div key={step.id} className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden mb-8">
          ...
          <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-center gap-4">
             <span className="bg-blue-100 text-blue-800 text-xs font-bold px-2 py-1 rounded">OP {step.step_number}</span>
             ...
             <form action={updateProcessStep} className="flex items-center gap-2">
                ...
                <input name="machine_tools" defaultValue={step.machine_tools || ''} placeholder="e.g. Injection M/C A" className="text-xs border border-gray-300 rounded p-1 w-48"/>
                ...
                {step.pfmea_records.map((risk: any) => (
                  <>
                    ...
                    <tr className="bg-blue-50 border-t border-blue-100">
                      <td className="hidden"><form id={`add-cp-${risk.id}`} action={addControlPlanRow}><input type="hidden" name="pfmea_id" value={risk.id} /><input type="hidden" name="project_id" value={id} /></form></td>
                      ...
                      <td className="p-1 text-center"><button type="submit" form={`add-cp-${risk.id}`} className="bg-blue-600 text-white text-[10px] px-2 py-1 rounded hover:bg-blue-500">ADD</button></td>
                    </tr>
                  </>
                ))}
              </tbody>
